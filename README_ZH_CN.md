...
- **🔍 智能搜索** — 支持分类搜索（仅语音 / 所有类型）、文件搜索（命名空间 6）和立绘预览模式。
...
- **🔍 四种搜索模式** — 仅语音、全部分类、文件搜索、立绘预览，通过底部导航栏切换。
- **⚡ 并发下载** — 与桌面端共用下载引擎，支持自定义并发数。
...
- **🎵 音频播放** — 支持播放 `WAV`、`OGG`、`MP3` 格式音频文件（本地与网络）。
- **🗂️ 文件选择对话框** — 按分类浏览文件，支持搜索、语言筛选（中/日/英）与图片预览。
- **⭐ 收藏功能** — 收藏常用角色，方便快速访问。
- **📶 网络检测** — 离线状态提示横幅，WebView 内置自定义错误页。
- **🎨 Material You** — 支持 Material 3 动态取色，亮色/暗色/跟随系统三种主题模式。
- **🔍 搜索历史** — 搜索记录以标签形式持久化展示。
