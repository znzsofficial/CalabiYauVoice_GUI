...
<div class="filters">
  <div class="filter-group"><span class="filter-label">范围</span><div class="chip-group">{#each PROFILES as profile (profile.value)}<button class:active={profile.value === activeProfile} class="chip" title={profile.desc} disabled={disabled} onclick={() => onSetProfile(profile.value)}>{profile.name}</button>{/each}</div></div>
